...
type Job_builder struct {
	...
	// one. On a job record this is the EFFECTIVE decision, the same sense as field
	// 21: false whenever the job's source declares no cache, whatever the request
	// said, because only a routed job can read content its authority never
	// confirmed and only a source with a cache is ever routed.
...
const file_gantry_job_proto_rawDesc = "" +
	...
	"\x04step\x18\v \x01(\rR\x04step\"\xe3\x06\n" +
	"\x03Job\x12\x19\n" +
