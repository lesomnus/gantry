...
| 4 | Platform selection | L1 `TestPlatformSelection`; narrowing to one platform commits the source's own child manifest — L1 `TestCommitNarrowedToOnePlatformCommitsTheChild`, `TestNarrowedCommitCarriesTheChildSignature`, routed in `TestRoutedEngineFillCarriesOnlyTheDeliveredPlatform`; **L2 `TestL2RoutedFillCarriesOnlyTheDeliveredPlatform`, `TestL2RoutedFillCarriesEveryPlatformWhenAsked`, `TestL2RegistryRejectsAnIndexMissingChildren` (why a filtered index is not an option at all)**; a digest `as` name keeps the narrowing and names the node after the index — L1 `TestNarrowedRouteNamesTheNodeAfterTheIndex`, `TestNarrowingUnderADigestNameNeedsNoPlatformSignature`, **L2 `TestL2NarrowedDigestNameRunsUnderEnforcement` (real daemon, enforcement on, index-only signature; pulls `docker.io/library/busybox` for a runnable child)** |
