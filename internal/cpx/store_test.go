...
func mkJob(id, ref string, platforms []string) *Job {
	...
	j.dedup = dedupKey(ref, platforms, "", "", nil, false, false, false)
	return j
}
...
func TestStoreActiveDedup(t *testing.T) {
	...
	_ = s.Add(a)
	key := dedupKey("img:1", []string{"linux/amd64", "linux/arm64"}, "", "", nil, false, false, false)
...
func TestFillingSkipsJobsThatCannotDeliver(t *testing.T) {
	...
	// A record built outside NewJob/setFills has no gate for the ref, so waiting
	// on it could only ever time out.
	s2 := NewMemStore()
	...
	if _, ok := s2.Filling(ref, ""); ok {
		t.Error("a job with no gate for the ref must not be offered to a waiter")
	}
}
